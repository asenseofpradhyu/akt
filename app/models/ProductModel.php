...
class ProductModel
{
	...
	{
		$query = "SELECT detail.product_name, detail.discount_price, ord.quantity, ord.size_id, s.title AS size, c.color, detail.product_code, img.images FROM product_orders INNER JOIN order_details AS ord ON product_orders.id=ord.order_id INNER JOIN product_detail AS detail ON ord.product_id = detail.product_id INNER JOIN color AS c ON ord.color_id = c.color_id JOIN sizes AS s ON ord.size_id=s.id LEFT JOIN image_data AS img ON ord.product_id=img.product_id AND ord.color_id=img.color_id WHERE product_orders.id= $orderId GROUP BY ord.product_id";
		$this->db->query($query);
	...
	{
		$query = "SELECT ship.*, p.purchase_json, p.purchase_amount, s.name AS state_name, c.name AS country_name FROM product_orders INNER JOIN shipping_info AS ship ON product_orders.shipping_address_id=ship.id INNER JOIN purchases AS p ON product_orders.purchase_id=p.id INNER JOIN states AS s ON ship.state_id=s.id INNER JOIN countries AS c ON ship.country_id=c.id WHERE product_orders.id= $orderId";
		$this->db->query($query);
