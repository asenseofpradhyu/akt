...
class ProductModel
{
  ...
  public function getCartProductSingleImg($id)
  {
    ...
    $this->db->query('SELECT cart.product_id, product_detail.product_name, product_detail.discount_price, product_detail.stock, color.color, image_data.images FROM image_data INNER JOIN product_detail ON product_detail.product_id = image_data.product_id INNER JOIN color ON color.color_id = image_data.color_id INNER JOIN cart ON cart.product_id = image_data.product_id WHERE cart.customer_id = :id GROUP BY image_data.product_id HAVING COUNT(image_data.product_id) > 1');
